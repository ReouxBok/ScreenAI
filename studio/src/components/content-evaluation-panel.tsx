...
export function ContentEvaluationPanel({ itemId, data, testCode, testCaseId }: { itemId: string; data: EvaluationData | null; testCode?: string; testCaseId?: string }) {
  ...
  return <section className="content-evaluation card" id="content-evaluation">
    ...
    <div className="evaluation-heading">
      <div><span className="eyebrow">Test facultatif · tout le flow</span><h2>Vérifier le parcours complet avec Charly</h2><p>Vous pouvez demander la validation immédiatement. Ce test optionnel permet de vérifier en conditions réelles la compréhension, les clics, la saisie, la navigation et le résultat final. Le brouillon est isolé et la conversation n’est pas mémorisée.</p></div>
    ...
    {!data ? <form action={prepareEvaluationAction}><input type="hidden" name="itemId" value={itemId}/><button className="primary" type="submit">Préparer le test complet</button></form> : <>
      ...
      {testCase && <div className="evaluation-cases"><article className="evaluation-case evaluation-case-single">
        ...
        <div><div className="evaluation-case-title"><strong>{testCase.title}</strong><span>Optionnel</span></div><p>Demandez à Charly d’atteindre l’objectif puis laissez-la réaliser l’ensemble du parcours.</p><small>Objectif : {testCase.expectation.objective} · Départ : {testCase.expectation.startPath}</small></div>
      ...
      <div className={`evaluation-gate ${data.suite.status}`}><strong>{data.suite.status === "passed" ? "Flow vérifié" : "Test qualité facultatif"}</strong><span>{data.suite.status === "passed" ? "Le flow complet a réussi sur cette version." : "Vous pouvez lancer ce test quand vous le souhaitez, sans bloquer la demande de validation."}</span></div>
    </>}
  </section>;
}
